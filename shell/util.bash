...
message() {
  ...
  printf "%b%s %s%b\n" "$ansi_format" "$prefix" "$message" "$(term_format reset)" >&2
}

# Accepts a message either via stdin or as the first argument. Does not exit.
info() {
  message '==>' "$(term_format bold)" "$@"
}

# Accepts a message either via stdin or as the first argument. Does not exit.
warn() {
  message 'WARNING:' "$(term_format yellow)" "$@"
}

# Accepts a message either via stdin or as the first argument. Does not exit.
fatal() {
  message 'FATAL:' "$(term_format red)" "$@"
}
...
# Dedents the given text (via stdin). Will not remove non-whitespace characters.
# Defaults to two spaces, takes optional argument for number of spaces to indent by.
dedent() {
  local num=${1:-2}
  perl -pe "s/^\\s{$num}//"
}
